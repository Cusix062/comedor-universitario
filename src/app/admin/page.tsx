    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-5 py-4 flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={async () => {
              if (!confirm("Enviar reporte de ALMUERZO a Telegram?")) return;
              try {
                const res = await fetch("/api/admin/telegram", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ fecha, turno: "almuerzo" }),
                });
                const data = await res.json();
                if (data.success) {
                  alert("Reporte de almuerzo enviado ✅");
                } else {
          ...
          <button
            onClick={async () => {
              if (!confirm("Enviar reporte de CENA a Telegram?")) return;
              try {
                const res = await fetch("/api/admin/telegram", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ fecha, turno: "cena" }),
                });
                const data = await res.json();
                if (data.success) {
                  alert("Reporte de cena enviado ✅");
                } else {
                  alert("Error al enviar: " + (data.error || "Desconocido"));
                }
              } catch {
