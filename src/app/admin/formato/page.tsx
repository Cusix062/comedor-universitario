              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3">
                <div className="flex gap-2 flex-wrap">
                  ...
                  <input type="number" value={capacidad} onChange={(e) => setCapacidad(parseInt(e.target.value) || 30)}
                    min="1" max="100" className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm w-16 text-center" />
                  <button onClick={guardarFormato} disabled={cargando || guardado}
                    className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${guardado ? "bg-gray-500" : "bg-emerald-600 hover:bg-emerald-700"} text-white disabled:opacity-50`}>
                    {guardado ? "✅" : cargando ? "..." : "💾"}
                  </button>
                  <button onClick={imprimir} disabled={!guardado}
                    className={`px-3 py-1.5 rounded-lg text-sm font-semibold ${guardado ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-gray-300 text-gray-500"}`}>
                    🖨️
                  </button>
                </div>
              </div>
